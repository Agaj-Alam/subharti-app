...
export default function Login() {
  ...
  const { token, loading, error } = useSelector((state: any) => state.auth); // ✅ CHANGED: user → token
  ...
  const handleLogin = () => {
    dispatch(loginUser({ enrollment: username, dob: password } as any));
  };

  // 🔥 REDIRECT AFTER LOGIN
  useEffect(() => {
    if (token) { // ✅ CHANGED: user → token
      router.replace("/(main)/(drawer)/(tabs)");
    }
  }, [token]); // ✅ CHANGED: dependency user → token
  ...
  return (
    ...
    >
      ...
      <View className="mx-5 rounded-3xl bg-white px-6 py-8">
        ...
        {/* Enrollment Input */}
        <TextInput
          placeholder="Enrollment No." // ✅ CHANGED label
          placeholderTextColor="#9ca3af"
          value={username}
          onChangeText={setUsername}
          className="mb-4 rounded-xl border border-gray-300 px-4 py-4 text-base"
        ...
        <View className="mb-6 flex-row items-center rounded-xl border border-gray-300 px-4">
          <TextInput
            placeholder="DOB (DDMMYY)" // ✅ CHANGED format
            placeholderTextColor="#9ca3af"
            secureTextEntry={!showPassword}
            value={password}
            onChangeText={setPassword}
